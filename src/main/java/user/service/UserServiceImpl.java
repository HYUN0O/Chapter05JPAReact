...
public class UserServiceImpl implements UserService {
	...
	@Override
	public String isExistId(String id) { /* 사용 가능인지 불가능인지 여기서 판단 */
		//DB
	...
	public void delete(String id) {
		...
		//delete()는 findById()를 수행하지 않고 바로 delete를 처리한다.
	...
	public Page<UserDTO> getUserSearchList(String columnName, String value, Pageable pageable) {
		if(columnName.equals("name")) { //select * from usertable where name like concat('%', '치', '%')
			return userDAO.findByNameContaining(value, pageable);
			
		} else { //if(columnName.equals("id")), select * from usertable where name id concat('%', 'c', '%')
			return userDAO.findByIdContaining(value, pageable);
		}
		
		//@Query 어노테이션
//		if(columnName.equals("name")) {
//			return userDAO.getUserSearchName(pageable, value);
//			
//		} else {
//			return userDAO.getUserSearchId(pageable, value);
//		}
